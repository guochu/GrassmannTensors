...
function LinearAlgebra.diagm(codom::TensorSpace, dom::TensorSpace, v::SectorDict)
...
function ⊗(t1::AbstractTensorMap, t2::AbstractTensorMap)
    ...
    for (f1l, f1r) in fusiontrees(t1)
        for (f2l, f2r) in fusiontrees(t2)
            ...
            c = c1 ⊗ c2 # unique fusion for FermionParity (Nsymbol inlined, μ = 1)
            # abelian merge: concatenate uncoupled sectors and isdual flags, coeff = 1
            fl = FusionTree((f1l.uncoupled..., f2l.uncoupled...), c,
                            (f1l.isdual..., f2l.isdual...))
            fr = FusionTree((f1r.uncoupled..., f2r.uncoupled...), c,
                            (f1r.isdual..., f2r.isdual...))
