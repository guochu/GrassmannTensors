...
spacetype(::Type{<:AbstractTensorMap}) = GradedSpace
sectortype(::Type{TT}) where {TT<:AbstractTensorMap} = FermionParity
...
hasblock(t::AbstractTensorMap, c::FermionParity) = c ∈ blocksectors(t)
...
function Base.similar(t::AbstractTensorMap, ::Type{TorA},
                      P::TensorMapSpace{N₁,N₂}) where {TorA,N₁,N₂}
    if TorA <: Number
        T = TorA
    ...
    return TensorMap{T,N₁,N₂,A,N₁+N₂}(undef, P)
end
