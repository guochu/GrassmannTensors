...
struct DiagonalTensorMap{T,A<:DenseVector{T}} <: AbstractTensorMap{T,1,1}
    data::A
    domain::GradedSpace # equals codomain

    # uninitialized constructors
    function DiagonalTensorMap{T,A}(::UndefInitializer,
                                    dom::GradedSpace) where {T,A<:DenseVector{T}}
...
# undef constructors
"""
    DiagonalTensorMap{T}(undef, domain::GradedSpace)
    # expert mode: select storage type `A`
    DiagonalTensorMap{T,A}(undef, domain::GradedSpace)

Construct a `DiagonalTensorMap` with uninitialized data.
"""
function DiagonalTensorMap{T}(::UndefInitializer, V::GradedSpace) where {T}
    return DiagonalTensorMap{T,Vector{T}}(undef, V)
end
DiagonalTensorMap(::UndefInitializer, V::GradedSpace) = DiagonalTensorMap{Float64}(undef, V)

function DiagonalTensorMap{T}(data::A, V::GradedSpace) where {T,A<:DenseVector{T}}
...
function DiagonalTensorMap{T}(f::Function, V::GradedSpace) where {T}
    data = f(T, reduceddim(V))
    return DiagonalTensorMap{T}(data, V)
end

function DiagonalTensorMap(data::DenseVector{T}, V::GradedSpace) where {T}
    return DiagonalTensorMap{T}(data, V)
end
...
#-------------------------------------------------
function block(d::DiagonalTensorMap, s::FermionParity)
...
function Base.getindex(iter::BlockIterator{<:DiagonalTensorMap}, c::FermionParity)
...
function Base.:/(t1::AbstractTensorMap, t2::DiagonalTensorMap)
    domain(t1) == domain(t2) ||
