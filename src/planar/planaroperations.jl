...
# 树级平面迹：把 f₁/f₂ 合并为单树（线性化顺序 = codomain 原序 + domain 逆序，同 permute），
# 校验并迹掉 (q₁′, q₂′) 配对的腿（Z₂ 中合法配对系数为 1），再按压缩后的 (p₁′, p₂′)
# 无符号地分裂回两棵树。对 FermionParity（UniqueFusion，Fsymbol 全 1）该结果是精确的。
function planar_trace(f₁::FusionTree, f₂::FusionTree,
                      p₁::IndexTuple{N₁}, p₂::IndexTuple{N₂},
                      q₁::IndexTuple{N}, q₂::IndexTuple{N}) where {N₁,N₂,N}
    N == 0 && return _treetranspose(f₁, f₂, p₁, p₂)
    f₁.coupled == f₂.coupled || throw(SectorMismatch())
    M₁, M₂ = length(f₁), length(f₂) # 输入树腿数（≠ 输出 N₁/N₂）
    linear = (ntuple(identity, M₁)..., reverse(M₁ .+ ntuple(identity, M₂))...)
    q₁′ = TupleTools.getindices(linear, q₁)
    q₂′ = TupleTools.getindices(linear, q₂)
    q′ = (q₁′..., q₂′...)
    uncoupled = (f₁.uncoupled..., reverse(f₂.uncoupled)...)
    isdual = (f₁.isdual..., reverse(map(!, f₂.isdual))...)
    # 每对被迹掉的腿必须是 dual 配对且 isdual 相反，否则迹为零（Z₂ 中 dual = 自身）
    for (i, j) in zip(q₁′, q₂′)
        (uncoupled[i] == uncoupled[j] && isdual[i] != isdual[j]) ||
            return emptyplanartrace()
    end
    # 剩余腿按原线性顺序保留；被迹对贡献单位 sector，耦合值不变
    remaining = TupleTools.deleteat(ntuple(identity, M₁ + M₂), q′)
    f′ = FusionTree(TupleTools.getindices(uncoupled, remaining), f₁.coupled,
                    TupleTools.getindices(isdual, remaining))
    # 压缩后的输出腿索引，用与 permute 相同的线性化-置换-分裂逻辑无符号地拆回两棵树
    p₁′ = map(l -> l - count(l .> q′), TupleTools.getindices(linear, p₁))
    p₂′ = map(l -> l - count(l .> q′), TupleTools.getindices(linear, p₂))
    p = linearizepermutation(p₁′, p₂′, N₁, N₂)
    f″ = FusionTree(TupleTools.getindices(f′.uncoupled, p), f′.coupled,
                    TupleTools.getindices(f′.isdual, p))
    uncoupled1 = ntuple(n -> f″.uncoupled[n], N₁)
    isdual1 = ntuple(n -> f″.isdual[n], N₁)
    uncoupled2 = ntuple(n -> f″.uncoupled[N₁ + n], N₂)
    isdual2 = map(!, ntuple(n -> f″.isdual[N₁ + n], N₂))
    c = couple(uncoupled1)
    f₁′ = FusionTree(uncoupled1, c, isdual1)
    f₂′ = FusionTree(uncoupled2, c, isdual2)
    return SingletonDict((f₁′, f₂′) => 1)
end

const _PLANAR_TRACE_EMPTY = FusionTreeDict{Tuple{FusionTree,FusionTree},Int}()
emptyplanartrace() = _PLANAR_TRACE_EMPTY
